...
with open('./sql/STG_Application.sql', 'r') as file:
    query = file.read()
...
try:
  deleteQuery = text('DELETE FROM staging."Application"')
  print(deleteQuery)
...
try:
    df.to_sql(
        name='Application',            # Replace with actual table name
        con=targetDB,
        schema='staging',            # 🔄 Specify schema here
        if_exists='append',          # Options: 'fail', 'replace', 'append'
        index=False
    )
    print("Data written to 'staging.Application' successfully.")
except Exception as e:
