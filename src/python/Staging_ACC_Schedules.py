...
with open('./sql/STG_ACC_Schedules.sql', 'r') as file:
    query = file.read()
...
try:
  deleteQuery = text('DELETE FROM staging."ACC_Schedules"')
  print(deleteQuery)
...
try:
    df.to_sql(
        name='ACC_Schedules',            # Replace with actual table name
        con=targetDB,
        schema='staging',            # 🔄 Specify schema here
        if_exists='append',          # Options: 'fail', 'replace', 'append'
        index=False
    )
    print("Data written to 'staging.ACC_Schedules' successfully.")
except Exception as e:
