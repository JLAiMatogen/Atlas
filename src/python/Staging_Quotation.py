...
with open('./sql/STG_Quotation.sql', 'r') as file:
    query = file.read()
...
try:
  deleteQuery = text('DELETE FROM staging."Quotation"')
  print(deleteQuery)
...
try:
    df.to_sql(
        name='Quotation',            # Replace with actual table name
        con=targetDB,
        schema='staging',            # 🔄 Specify schema here
        if_exists='append',          # Options: 'fail', 'replace', 'append'
        index=False
    )
    print("Data written to 'staging.Quotation' successfully.")
except Exception as e:
