...
with open('./sql/STG_CreditScore.sql', 'r') as file:
    query = file.read()
...
try:
  deleteQuery = text('DELETE FROM staging."CreditScore"')
  print(deleteQuery)
...
try:
    df.to_sql(
        name='CreditScore',            # Replace with actual table name
        con=targetDB,
        schema='staging',            # 🔄 Specify schema here
        if_exists='append',          # Options: 'fail', 'replace', 'append'
        index=False
    )
    print("Data written to 'staging.CreditScore' successfully.")
except Exception as e:
