...
with open('./sql/STG_Affordability.sql', 'r') as file:
    query = file.read()
...
try:
  deleteQuery = text('DELETE FROM staging."Affordability"')
  print(deleteQuery)
...
try:
    df.to_sql(
        name='Affordability',            # Replace with actual table name
        con=targetDB,
        schema='staging',            # 🔄 Specify schema here
        if_exists='append',          # Options: 'fail', 'replace', 'append'
        index=False
    )
    print("Data written to 'staging.Affordability' successfully.")
except Exception as e:
