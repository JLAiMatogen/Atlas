...
ACC_DebitOrder as materialized (
	select d.* from (
		...
		from 		ACC_Account a, backoffice.public."ACC_DebitOrder" ado 
...
Application as (
	...
	from 	ACC_Account ac 
					...
					left outer join backoffice.sqlmig."Client" c
						on  a."ApplicationClientId"   = c."ClientId"
...
FinalLoanDetail as materialized (
	select  ld."HandedOver" "HandedOver_Ind"
		...
		, 	case 
				when ( ld."IsDue" = 1 and ld."IsLate7Days" = 1 ) then ld."Duedate" 
				when ( ld."IsDue" = 1 and ld."Installment_Paid_Perc" < 85 ) then ld."Duedate"
				else null
				end  "ArrearDate"
		,  	ld."PaymentFrequency"
		,   ld."Loan_Size"
